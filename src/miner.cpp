...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BarthorMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BarthorMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BarthorMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("barthor-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BarthorMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
